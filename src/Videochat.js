...
function VideoChat() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('chat_message', ({ message }) => {
      setMessages(prev => [...prev, { text: message, from: 'stranger' }]);
    });
  ...
  const startPeerConnection = async (initiator) => {
    ...
    peer.on('signal', (data) => {
      if (data.type === 'offer') {
        socketRef.current.emit('offer', { offer: data });
      } else if (data.type === 'answer') {
        socketRef.current.emit('answer', { answer: data });
      } else {
        socketRef.current.emit('ice_candidate', { candidate: data });
      }
    });
